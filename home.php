...
<div class="about" id="about" style="background: url(<?php echo CFS()->get('bg_white') ?>) center 100% repeat-x, url(<?php echo CFS()->get('bg_black') ?>) center 100% repeat-x, #1d1d1d;">
    ...
    <div class="contact" id="contact">
        <div class="container">
            <div class="block__head">
                <h2 class="block__title"><?php echo CFS()->get('contac_title') ?></h2>
                <p class="block__text"><?php echo CFS()->get('contac_tagline') ?></p>
            </div>
            <div class="contact__inner">
                <div class="contact__icon-box">
                    <div class="contact__item">
                        <i class="icon-phone"></i>
                        <div class="contact__text"><a href="tel:<?php echo CFS()->get('contac_phone') ?>"><?php echo CFS()->get('contac_phone') ?></a></div>
                    </div>
                    <div class="contact__item">
                        <?php 
                        $address = CFS()->get('contac_address')
                        ?>
                        <i class="icon-address-book"></i> 
                        <div class="contact__text">
                            <a href="<?php echo $address['url'] ?>" target="<?php echo $address['target'] ?>">Here is some address</a>
                        </div>
                    </div>
                    <div class="contact__item">
                        <i class="icon-mail-alt"></i>
                        <div class="contact__text"><a href="mailto:<?php echo CFS()->get('contac_email') ?>"><?php echo CFS()->get('contac_email') ?></a></div>
                    </div>
                </div>
                <?php the_content(); ?>
            </div>
